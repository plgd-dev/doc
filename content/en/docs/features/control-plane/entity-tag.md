...
- **ETAG**: The ETAG is a 8byte opaque value that is generated by the device and used to detect changes in resources.
- **Global ETAG**: Represents latest and highest generated ETAG value among all resources in the device. It is used to generate next ETAG when a resource is created, or modified.
- **Collision**: Occurs when the ETAG value is the same for the resource with different content. This can happen when the device is reset and the global ETAG value is the same as the ETAG value of the resource that was changed by the client.
...
3. Otherwise, increment the global ETAG value by one and use it as the resource ETAG for the changed resource.
4. Set the generated ETAG from the step `3.` to the changed resource as resource ETAG.
...
**Collision example**: The device reset always time to 1.1.1970 because it does not have a real-time clock and it contains only one modifiable resource. The device is running with updated the resource with ETAG 1000 and client has also ETAG 1000. After that, the device loses power (none etags has been stored in persistent storage) and started but unfortunately random value returns 0 as previous run. Another client make update the resource in same time as in previous run, but it changes another property and after that the ETAG is 1000. The first client makes a request to the resource and receives a response with code `VALID` and the resource content from the second client. The first client does not know that the resource was changed by another client and it has the same ETAG value. But the resource content is different.
