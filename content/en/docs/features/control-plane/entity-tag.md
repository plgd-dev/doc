...
- **Collision**: A collision happens when a client has a cached resource with a specific ETAG value, but the device has a same ETAG associated with the different resource state. Collisions can occur when there is a lack of synchronization between the client and the device's time.
